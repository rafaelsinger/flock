...
interface HeaderProps {
  /**
   * Keeps header background transparent until user scrolls
   */
  transparent?: boolean;
  /**
   * Toggle the temporary service‑status banner. Comment this out or set to `false` when not needed.
   */
...
export const Header = ({ transparent = false, showStatusBanner = false }: HeaderProps) => {
  ...
  return (
    ...
    >
      {/* ------------------------------------------------------------------
        Temporary service‑status banner
        Comment out the block below or pass `showStatusBanner={false}` to <Header />
        when the server is back online.
      ------------------------------------------------------------------- */}
      {showStatusBanner && (
        <div className="w-full bg-amber-100 text-amber-800 text-center text-xs sm:text-sm py-2 px-4 mb-1 rounded-md">
          We’re currently experiencing server issues—our directory will be back up shortly.
        </div>
      )}
